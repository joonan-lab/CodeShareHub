...
run_ORA <- function(df, df_source = NA, organism = NA, change = NA, lfc_thresh = 0.25, p.adj_thresh = 0.05, outtag = NA){
  if(is.na(df_source)){
    stop('argument "df_source" is missing, supports Seurat or DESeq2')
  }
  if(is.na(change)){
  ...
  if(change == 'pos'){
    cat(paste('Extracting Up-regulated genesets with abs(log2FC) > ', lfc_thresh, ' & p.adj < ', p.adj_thresh,  '\n** Users can change cutoffs by adjusting the options "lfc_thresh" or "p.adj_thresh". \n\n', sep = ''))
  } else if (change == 'neg'){
    cat(paste('Extracting Down-regulated genesets with abs(log2FC) > ', lfc_thresh, ' & p.adj ', p.adj_thresh,  '\n** Users can change cutoffs by adjusting the options "lfc_thresh" or "p.adj_thresh". \n\n', sep = ''))
  } else {
    stop('argument "change" must be pos or neg')
  }
  ...
  if(df_source == 'DESeq2'){
    ...
    stop('argument "df_source" must be DESeq2 or Seurat')
  }
