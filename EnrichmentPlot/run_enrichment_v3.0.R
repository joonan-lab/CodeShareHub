...
run_all <- function(df, 
  ...
  res_list = list(DEGs = prep_df, ORA_up = ORA_res[['Up']], ORA_down = ORA_res[['Down']], GSEA = GSEA_res)
...
test_res = run_all(deg_res, lfc = 'log2FoldChange',padj = 'padj',
                   gene_name = 'gene_name', lfc_thresh = 0.25, padj_cutoff = 0.05,
                   organism = 'human')

##### Visualization #####
plot_volcano <- function(df, condition = NA){
